...
public class ContentsProc implements ContentsProcInter {
  ...
  public ArrayList<ContentsVO> list_by_cafeno(int cafeno) {
    ArrayList<ContentsVO> list = this.contentsDAO.list_by_cafeno(cafeno);
    return list;
  }
  ...
  public ContentsVO read(int cafe_contentsno) {
    ContentsVO contentsVO = this.contentsDAO.read(cafe_contentsno);
    return contentsVO;
  }
  ...
  public ArrayList<ContentsVO> list_by_cafeno_search(HashMap<String, Object> hashMap) {
    ArrayList<ContentsVO> list = this.contentsDAO.list_by_cafeno_search(hashMap);
    return list;
  }

  @Override
  public int list_by_cafeno_search_count(HashMap<String, Object> hashMap) {
    int cnt = this.contentsDAO.list_by_cafeno_search_count(hashMap);
    return cnt;
  }
  ...
  public ArrayList<ContentsVO> list_by_cafeno_search_paging(HashMap<String, Object> map) {
    ...
    ArrayList<ContentsVO> list = this.contentsDAO.list_by_cafeno_search_paging(map);
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성
      str.append("<span class='span_box_1'><A href='" + list_file + "?cafeno="+cafeno+"&word=" + word + "&now_page=" + _now_page
          + "'>이전</A></span>");
    }
    ...
    for (int i = start_page; i <= end_page; i++) {
      ...
      if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='" + list_file + "?cafeno="+cafeno+"&word=" + word + "&now_page=" + i + "'>" + i
            + "</A></span>");
      }
    }
  ...
  public int delete(int cafe_contentsno) {
    int cnt = this.contentsDAO.delete(cafe_contentsno);
    return cnt;
  }

  @Override
  public int count_by_cafeno(int cafeno) {
    int cnt = this.contentsDAO.count_by_cafeno(cafeno);
    return cnt;
  }

  @Override
  public int delete_by_cafeno(int cafeno) {
    int cnt = this.contentsDAO.delete_by_cafeno(cafeno);
    return cnt;
  }

  @Override
  public int count_by_cafe_mno(int cafe_mno) {
    int cnt = this.contentsDAO.count_by_cafe_mno(cafe_mno);
    return cnt;
  }

  @Override
  public int delete_by_cafe_mno(int cafe_mno) {
    int cnt = this.contentsDAO.delete_by_cafe_mno(cafe_mno);
    return cnt;
  }
